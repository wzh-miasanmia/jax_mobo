...
class WENO6CUM2(SpatialReconstruction):
    ...
    def __init__(
        ...
        super(WENO6CUM2, self).__init__(nh=nh, inactive_axes=inactive_axes, offset=offset)
    ...
    def reconstruct_xi(
        ...
        alpha_0 = self.dr_[0] * jnp.power( ( self.Cq_ + tau_6 / (beta_0 + self.eps_ * dx2) * (beta_ave + self.chi * dx2) / (beta_0 + self.chi * dx2) ), self.q_ )
        alpha_1 = self.dr_[1] * jnp.power( ( self.Cq_ + tau_6 / (beta_1 + self.eps_ * dx2) * (beta_ave + self.chi * dx2) / (beta_1 + self.chi * dx2) ), self.q_ )
        alpha_2 = self.dr_[2] * jnp.power( ( self.Cq_ + tau_6 / (beta_2 + self.eps_ * dx2) * (beta_ave + self.chi * dx2) / (beta_2 + self.chi * dx2) ), self.q_ )
        alpha_3 = self.dr_[3] * jnp.power( ( self.Cq_ + tau_6 / (beta_3 + self.eps_ * dx2) * (beta_ave + self.chi * dx2) / (beta_3 + self.chi * dx2) ), self.q_ )
