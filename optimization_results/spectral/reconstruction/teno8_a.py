...
class TENO8A(SpatialReconstruction):
    ...
    def __init__(self, 
            ...
            offset: int = 0,
            **kwargs) -> None:
        super(TENO8A, self).__init__(nh=nh, inactive_axes=inactive_axes, offset=offset)
    ...
    def reconstruct_xi(self, 
        ...
        # eta_{i-1} -> (u_{i-2}, u_{i-1}, u_{i})
        eta_0 = (jnp.abs(2 * (buffer[s1_[3]] - buffer[s1_[2]]) * (buffer[s1_[2]] - buffer[s1_[1]])) + self.eps_dissipation) \
            / ((buffer[s1_[3]] - buffer[s1_[2]])**2 + (buffer[s1_[2]] - buffer[s1_[1]])**2 + self.eps_dissipation) 
        # eta_{i}   -> (u_{i-1}, u_{i}, u_{i+1})
        eta_1 = (jnp.abs(2 * (buffer[s1_[4]] - buffer[s1_[3]]) * (buffer[s1_[3]] - buffer[s1_[2]])) + self.eps_dissipation) \
            / ((buffer[s1_[4]] - buffer[s1_[3]])**2 + (buffer[s1_[3]] - buffer[s1_[2]])**2 + self.eps_dissipation)
        # eta_{i+1} -> (u_{i}, u_{i+1}, u_{i+2})
        eta_2 = (jnp.abs(2 * (buffer[s1_[5]] - buffer[s1_[4]]) * (buffer[s1_[4]] - buffer[s1_[3]])) + self.eps_dissipation) \
            / ((buffer[s1_[5]] - buffer[s1_[4]])**2 + (buffer[s1_[4]] - buffer[s1_[3]])**2 + self.eps_dissipation)
        # eta_{i+2} -> (u_{i+1}, u_{i+2}, u_{i+3})
        eta_3 = (jnp.abs(2 * (buffer[s1_[6]] - buffer[s1_[5]]) * (buffer[s1_[5]] - buffer[s1_[4]])) + self.eps_dissipation) \
            / ((buffer[s1_[6]] - buffer[s1_[5]])**2 + (buffer[s1_[5]] - buffer[s1_[4]])**2 + self.eps_dissipation)
        ...
        delta_4 = jnp.where(gamma_4 * one_gamma_sum < CT, 0, 1)
        delta_5 = jnp.where(gamma_5 * one_gamma_sum < CT, 0, 1)
