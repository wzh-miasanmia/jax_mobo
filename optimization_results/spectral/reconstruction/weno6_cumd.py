...
class WENO6CUMD(SpatialReconstruction):
    ...
    def __init__(self, 
            ...
            offset: int = 0,
            **kwargs) -> None:
        super(WENO6CUMD, self).__init__(nh=nh, inactive_axes=inactive_axes, offset=offset)
        ...
        self.array_slices([
            range(-3, 3, 1), range(2, -4, -1)
        ])

        self.stencil_slices([
            range(0, 6, 1), range(5, -1, -1)
        ])
    ...
    def reconstruct_xi(self, buffer: jnp.ndarray, axis: int, j: int, dx: float, **kwargs) -> jnp.ndarray:
        ...
        alpha_0 = self.dr_[0] * jnp.power( (self.C_ + tau_6 / (beta_0 + self.eps) + dx**(1/2) * beta_0 / (tau_6 + self.eps)), self.q_ )
        alpha_1 = self.dr_[1] * jnp.power( (self.C_ + tau_6 / (beta_1 + self.eps) + dx**(1/2) * beta_1 / (tau_6 + self.eps)), self.q_ )
        alpha_2 = self.dr_[2] * jnp.power( (self.C_ + tau_6 / (beta_2 + self.eps) + dx**(1/2) * beta_2 / (tau_6 + self.eps)), self.q_ )
        alpha_3 = self.dr_[3] * jnp.power( (self.C_ + tau_6 / (beta_3 + self.eps) + dx**(1/2) * beta_3 / (tau_6 + self.eps)), self.q_ )
