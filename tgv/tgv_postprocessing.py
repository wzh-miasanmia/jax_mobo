...
H5_FILE = "results/tgv-1/domain/data_*.h5"
TITLE = "WENO6-CUM1 $C_q=1000, q=1, Re=1000$"
...
def calculate_ftke(file):
    ...
    R_tke_sample = np.sum((np.log(spectrum_data[3:34, 1]) - np.log(spectrum_ref[3:34])) ** 2) / len(spectrum_data[3:34, 1])

    Rtke_baseline = 3.0398569829217728 # read from baseline
    ftke = R_tke_sample / Rtke_baseline - 1
    return ftke

def calculate_cell_volume(file):
...
def calculate_fcons(file):
    cell_volume = calculate_cell_volume('tgv/tgv.json')
    kinetic_energy = calculate_kinetic_energy(file, cell_volume)
    file = get_file_list(file)
    kinetic_energy_sample = calculate_kinetic_energy(file, cell_volume)
    print(kinetic_energy_sample)
    kinetic_energy_baseline = 18.289785674371107 # read from baseline folder
    fcons = - (kinetic_energy_sample / kinetic_energy_baseline - 1) # we want energy to become larger and fcons to minimize
    return fcons 
