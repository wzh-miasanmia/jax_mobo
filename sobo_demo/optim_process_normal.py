...
def optim_process(
    ...
    for idx in range(n_init, n + n_init):
        ...
        params, momentums, scales  = gp.optimize_mll(params, X_normalized, Y, momentums, scales, noise, num_steps, lr)
        momentums = tree_map(lambda x: x * 0, params)
        scales = tree_map(lambda x: x * 0 + 1, params)
        l_opt, sigma_f_opt = params.lengthscale, params.amplitude

        # Obtain next sampling point from the acquisition function(expected_improvement)
        next_X_normal, key = suggest_next(
            key, 
            X_normalized, 
            Y, 
            bounds_normal, 
        ...
        next_X = unnormalize(next_X_normal, bounds)
