...
def optim_process(
    ...
    Y_sample_multi = np.array([select_para_and_simulation(para=p, 
                                                  ...
                                                  DEBUG=DEBUG) for p in X_init]) # jax-fluid simulation
    ## TODO: check the dimension: [n_f, n_samples, 1]
    n_f = Y_sample_multi.shape[0] # number of functions
    gpr_list = [GaussianProcessRegressor(sigma_y=noise) for _ in range(n_f)]

    # Initialize samples, retain 4 decimal places
    X_sample = np.round(X_init, 4)
    Y_sample_multi = np.round(Y_sample_multi, 4)
