...
class Node:
    ...
    def __init__(
        ...
        self.prev = [None] * m
        self.ignore = 0
        self.area = jnp.zeros(m, dtype=dtype)
        self.volume = jnp.zeros_like(self.area)
...
class MultiList:
    ...
    def __init__(self, m: int, dtype: jnp.dtype) -> None:
    ...
    def remove(self, node: Node, index: int, bounds: jnp.ndarray) -> Node:
        ...
        bounds[...] = jnp.minimum(bounds, node.data)
        return node

    def reinsert(self, node: Node, index: int, bounds: jnp.ndarray) -> None:
...
class Hypervolume:
    ...
    def compute(self, pareto_Y):
        ...
        bounds = jnp.full_like(self._ref_point, float("-inf"))
    ...
    def plot(self, ref_point, pareto_Y):
        m = pareto_Y.shape[1]
        bounds = jnp.full_like(ref_point, float("-inf"))
        self._initialize_multilist(pareto_Y)
