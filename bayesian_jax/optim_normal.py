...
from bayesian_jax.gp import DataTypes, GParameters, round_integers, train, predict
...
def optim(
    ...
    **acq_params: dict,
    ...
    # normalization
    bounds_normal = jnp.array([[0.0, 1.0]] * dim)

    for idx in range(n_init, n + n_init):
        X_normalized = normalize(X, bounds)
        params, momentums, scales = train(
            X_normalized, Y, params, momentums, scales, dtypes
        )
        max_params_normal, key = suggest_next(key, params, X_normalized, Y, bounds_normal, dtypes, _acq)
        max_params = unnormalize(max_params_normal, bounds)
